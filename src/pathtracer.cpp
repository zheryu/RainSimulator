...
PathTracer::PathTracer(size_t ns_aa,
  ...
  gridSampler = new JitteredGridSampler2D((int) ceil(sqrt(ns_aa)));
...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  for(SceneLight *l : scene->lights){
    ...
    Spectrum sample_aggregate = Spectrum();
    for(int i = 0; i < num_samples; i++){
      Vector3D wi;
      float distToLight, pdf;
      Spectrum L = l->sample_L(hit_p, &wi, &distToLight, &pdf);
      Vector3D w_in = w2o * wi;
      if(w_in.z >= 0 && !(bvh->intersect(Ray((EPS_D * wi) + hit_p, wi, distToLight), bvh->get_root()))){
        sample_aggregate += L * abs_cos_theta(w_in) * isect.bsdf->f(w_out, w_in) / pdf;
      }
    }
    L_out += sample_aggregate/num_samples;
  }
  return L_out;
}
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  float tpdf = clamp(sample.illum()*10, 0, 1);
  if(coin_flip(1 - tpdf)){
    return Spectrum();
  }else{
    Vector3D  hit_p = r.o + r.d * isect.t,
              wi = o2w * w_in;
    Spectrum L = trace_ray(Ray((EPS_D * wi) + hit_p, wi, INF_D, r.depth - 1), isect.bsdf->is_delta());
    return L * abs_cos_theta(w_in) * sample / (pdf * tpdf);
  }
}
...
Spectrum PathTracer::trace_ray(const Ray &r, bool includeLe) {
  ...
  // Delta BSDFs have no direct lighting since the y are zero with probability 1 --
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if(num_samples == 1){
    Ray camRay = camera->generate_ray((x + 0.5) / sampleBuffer.w, (y + 0.5) / sampleBuffer.h);
    camRay.depth = max_ray_depth;
    return trace_ray(camRay, true);
  }
  ...
  for(int i = 0; i < num_samples; i++){
    Vector2D sampCoord;
    JitteredGridSampler2D* jitteredSampler = dynamic_cast<JitteredGridSampler2D*>(gridSampler);
    if(jitteredSampler){
      sampCoord = jitteredSampler->get_sample(i) + origin;
    }else if(dynamic_cast<UniformGridSampler2D*>(gridSampler)){
      sampCoord = gridSampler->get_sample() + origin;
    }
    Ray camRay = camera->generate_ray(sampCoord.x / sampleBuffer.w, sampCoord.y / sampleBuffer.h);
    camRay.depth = max_ray_depth;
    s += trace_ray(camRay, true);
  }
