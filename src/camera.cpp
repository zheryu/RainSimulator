...
void Camera::set_aperture_size(const double apertureW, const double apertureH) {
  this->apertureW = apertureW;
...
Ray Camera::generate_ray(double x, double y) const {
  ...
  Vector3D d = (c2w*Vector3D(topRight.x * x, topRight.y * y, -1)).unit();
  
  Ray r = Ray(pos, d, fClip);
  r.min_t = nClip;
  return r;
